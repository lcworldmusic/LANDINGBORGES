...
const ServiceTier: React.FC<ServiceTierProps> = ({ title, description, features, icon, href, recommended }) => (
  <div className={`relative p-8 border flex flex-col h-full transition-all group hover:-translate-y-2 ${
    recommended 
      ? 'bg-neutral-900 border-[#4682B4] shadow-[0_0_30px_rgba(70,130,180,0.1)] z-10' 
      : 'bg-black border-neutral-800 hover:border-neutral-700'
  }`}>
    {recommended && (
      <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#B8860B] text-black text-xs font-black px-4 py-1.5 uppercase tracking-widest skew-x-[-10deg]">
        <span className="skew-x-[10deg] inline-block font-oswald tracking-widest">Recomendado</span>
      </div>
    )}
    
    <div className="mb-8 border-b border-neutral-800 pb-8">
      <div className={`w-14 h-14 flex items-center justify-center mb-6 border ${
        recommended ? 'bg-[#4682B4] text-white border-[#4682B4]' : 'bg-neutral-900 text-white border-neutral-800'
      }`}>
        {icon}
      </div>
    ...
    <ul className="space-y-4 mb-10 flex-1">
      {features.map((feature, idx) => (
        <li key={idx} className="flex items-start gap-3 text-sm text-neutral-300 font-medium">
          <Check className={`w-5 h-5 shrink-0 ${recommended ? 'text-[#B8860B]' : 'text-neutral-600'}`} />
          {feature}
        </li>
      ))}
    </ul>
    ...
    <a 
      ...
      className={`w-full py-4 text-center font-bold uppercase tracking-widest transition-all skew-x-[-10deg] ${
        recommended 
          ? 'bg-[#4682B4] hover:bg-[#5A9BD5] text-white' 
...
export const Services: React.FC = () => {
  return (
    <div className="w-full relative">
       {/* Background accent */}
       <div className="absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-[#4682B4] to-transparent opacity-50"></div>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-20 space-y-4">
          ...
          <h2 className="text-5xl md:text-7xl font-bold text-white uppercase tracking-tighter font-display">
            ACCEDE A LA <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#0D3A5C] from-60% to-[#7A5A20]">MÁQUINA</span>
          </h2>
          <p className="text-[#B8860B] font-display text-xl uppercase tracking-[0.2em] font-medium">
            Recomendaciones de un artista
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          <ServiceTier 
            ...
            href="https://forms.gle/2u6LNotGg6gYuCom8"
            features={[
          ...
          <ServiceTier 
            ...
            recommended={true}
            href="https://forms.gle/UHv4aLaj7bDGYGDQ6"
            features={[
          ...
          <ServiceTier 
            ...
            href="https://forms.gle/qTGQmHZYuD1N1vry9"
            features={[
